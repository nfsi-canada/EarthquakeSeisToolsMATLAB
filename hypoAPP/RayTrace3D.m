function [tpp,RP] = RayTrace3D(x0,x1,FV,minV,maxV,minZ)
% function [tpp,RP] = RayTrace3D(x0,x1,FV,minV,maxV,minZ)
%
% 2020-06-12
...
%                but you can add station elevation
...
%                [x,y,z] points of that ray-path.
%                note that z == depth (positive==down)
%
...
% -- If desired you can give an intial ray-path estimate
% -- ...if not, a simple 3-point linear path is created
%if nargin < 4
%    RP   = cell(Nray,1); 
%    for ii = 1:Nray
%        % -- Initialize at 3 point ray-path
%        wX     = repmat(linspace(0,1,3)',1,3);
%        RP{ii} = flipud(wX).*repmat(x0(ii,:),3,1)+wX.*repmat(x1(ii,:),3,1);
%    end
%end
