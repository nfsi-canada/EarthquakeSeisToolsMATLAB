function [hyp1,T1,stats] = hypoDD(hyp0,T0,sta,phadir,mct,mcc,msp,model,params)
% function [hyp1,T1,stats] = hypoDD(hyp0,T0,sta,phadir,mct,mcc,msp,model,params)
%
% 2020-01-18
% This code mimics "hypoDD" by Felix Waldhauser.
% Some comments refer to Waldhauser and Ellsworth (2000,BSSA) as W2000.
% This is the "sP" version, which uses sP - P times to constrain 
% hypocenter depths
%
%   INPUTS 
%
...
%      msp = [EV,STA,TsP] list of sP - P times
...
disp(' ')
disp(['%%%%%%%%%%%%%%   hypoDDsP starting ',datestr(datetime('now')),'   %%%%%%%%%%%%%%'])
...
for iter = 1:params.Niter
    ...
    [GsP,dsP] = sPmatrixHypoDD(msp,H,S,FZsp,params.wSP(iter),jE);
    
    % -- Form zero-shift  and damping matrices and RHS vectors
    [GZ,dZ] = ZeroShiftHypoDD(C,params.W00,evsSP);
...
disp(['hypoDDsP complete ',datestr(datetime('now'))])
